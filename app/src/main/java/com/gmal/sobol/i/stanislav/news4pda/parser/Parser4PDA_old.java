...
public class Parser4PDA_old implements Parser4PDAViewable_old {

    private NewsDTO_old news = new NewsDTO_old();
    private DetailedNewDTO_old detailedNew = new DetailedNewDTO_old();
    private SQLiteManagerDataProvider sqLiteManagerDataProvider;

    public Parser4PDA_old() {
    ...
    public NewsDTO_old getParsedNewsData() {
        return news;
    }
    ...
    @Override
    public DetailedNewDTO_old getParsedDetailedNewData() {
        return detailedNew;
    }
    ...
    synchronized private void parseDetailedNewDocument(Document document) {
        ...
        for (Element p : ps) {
            DetailedNewDTO_old.ContentItem_old contentItemOld = new DetailedNewDTO_old.ContentItem_old();

            if (p.attr("style").equals("text-align: justify;")) {
                contentItemOld.isImage = false;
                contentItemOld.content = p.text();
            } else if (p.attr("style").equals("text-align: center;")) {
                contentItemOld.isImage = true;
                try {
                    contentItemOld.content = p.getElementsByTag("a").get(0).getElementsByTag("img").attr("src");
                } catch (java.lang.IndexOutOfBoundsException e) {
                    e.printStackTrace();
                    // TODO To the error report
                    contentItemOld.content = "";
                }
            }

            detailedNew.add(contentItemOld);
        }
    }
